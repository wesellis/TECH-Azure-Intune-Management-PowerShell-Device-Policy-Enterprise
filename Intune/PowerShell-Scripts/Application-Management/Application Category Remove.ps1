...
<#
.SYNOPSIS
    We Enhanced Application Category Remove
...
.EXAMPLE
Get-ApplicationCategory -ErrorAction Stop
Returns any application categories configured in Intune
.NOTES
NAME: Get-ApplicationCategory -ErrorAction Stop
...
$WEResource = " deviceAppManagement/mobileAppCategories"
...
.NOTES
NAME: Remove-ApplicationCategory -ErrorAction Stop
...
$WEResource = " deviceAppManagement/mobileAppCategories"
    ...
    try {
        ...
        Write-Information " No id specified for application category, can't remove application category..." -f Red
        Write-Information " Please specify id for application category..." -f Red
        break
...
; 
$WEApp = Get-ApplicationCategory -Name " LOB Apps 2" | Where-Object { $_.lastModifiedDateTime -ne " 0001-01-01T00:00:00Z" }

    if($WEApp){

        if(@($WEApp).count -gt 1){

        Write-WELog " More than one Application Categrory has been found, please specify a single Application Category..." " INFO" -ForegroundColor Red
        Write-Information }

        elseif(@($WEApp).count -eq 1){

        Write-WELog " Removing Application Category" " INFO" $WEApp.displayName -ForegroundColor Yellow
        Remove-ApplicationCategory -id $WEApp.id
    ...
    Write-WELog " Application Category can't be found or its an inbuilt category name that can't be removed..." " INFO" -ForegroundColor Yellow
    Write-Information }
