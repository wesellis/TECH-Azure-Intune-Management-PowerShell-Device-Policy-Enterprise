...
<#
.SYNOPSIS
    We Enhanced Application Mdm Remove
...
.NOTES
NAME: Remove-IntuneApplication -ErrorAction Stop
...
$WEResource = " deviceAppManagement/mobileApps"
    ...
    try {
        ...
        Write-Information " No id specified for application, can't remove application..." -f Red
        Write-Information " Please specify id for application..." -f Red
        break
...
; 
$WEApp = Get-IntuneApplication -Name " Microsoft Excel"

if($WEApp){

    if(@($WEApp).count -gt 1){

    Write-WELog " More than one application has been found, please specify a single application..." " INFO" -ForegroundColor Red
    Write-Information }

    elseif(@($WEApp).count -eq 1){

    Write-Information " Removing Application..." -f Yellow
    $WEApp.displayname + " : " + $WEApp.'@odata.type'
    $WEApp.id
    Remove-IntuneApplication -id $WEApp.id
    Write-Information }
...
Write-WELog " Application doesn't exist in Intune..." " INFO" -ForegroundColor Red
Write-Information }
