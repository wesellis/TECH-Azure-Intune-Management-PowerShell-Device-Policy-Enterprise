...
<#
.SYNOPSIS
    We Enhanced Intune Remediation Bios Thermalmode Setting
...
try
{
    ...
    $WEBIOSThermalMode= Switch ( Get-CimInstance -Namespace root/dcim/sysman -ClassName DCIM_ThermalInformation -Filter " AttributeName='Thermal Mode'" | Select-Object -ExpandProperty CurrentValue)
    {
        0 {" Optimized" }
        1 {" Cool" }
        2 {" Quiet" }
    ...
    else
    {
        ...
        elseif($WECheckAdminPW -match " 1" )
        {
            ...
            if($WERegKeyexist -eq " True" )
		    {
                ...
                $WEPWKey = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Dell\BIOS\' -Name BIOS | Select-Object -ExpandProperty BIOS
       ...
       ;  $WEThermalModeSetStatus = Get-CimInstance -Namespace root\dcim\sysman -ClassName DCIM_ThermalInformation | Invoke-CimMethod -MethodName ChangeThermalMode -Arguments @{AttributeName=@(" Thermal Mode" );AttributeValue=@(" 2" );AuthorizationToken=$WEAdminPw}

        if($WEThermalModeSetStatus[0].SetResult[0] -eq 0)
        {
            Write-WELog " BIOS Thermal Mode is set to 'Quiet' mode." " INFO"
            exit 0
        }
        elseif($WEThermalModeSetStatus[0].SetResult[0] -eq 2)
        {
