...
<#
.SYNOPSIS
    We Enhanced Managedapppolicy Add Assign
...
.EXAMPLE
Add-ManagedAppPolicy -JSON $WEJSON
...
$WEResource = " deviceAppManagement/managedAppPolicies"
    ...
    try {
        ...
        Write-Information " No JSON specified, please specify valid JSON for a Managed App Policy..." -f Red
...
.EXAMPLE
Assign-ManagedAppPolicy -Id $WEId -TargetGroupId $WETargetGroupId -OS Android
Assigns an AAD Group assignment to an Android App Protection Policy in Intune
.EXAMPLE
Assign-ManagedAppPolicy -Id $WEId -TargetGroupId $WETargetGroupId -OS iOS
...
" @
        ...
        Write-Information " No OS parameter specified, please provide an OS. Supported value Android or iOS..." -f Red
        Write-Information break
        ...
        $uri = " https://graph.microsoft.com/beta/deviceAppManagement/iosManagedAppProtections('$WEID')/assign"
        ...
        $uri = " https://graph.microsoft.com/$graphApiVersion/deviceAppManagement/iosManagedAppProtections('$WEID')/assign"
        Invoke-RestMethod -Uri $uri -Method Post -ContentType " application/json" -Body $WEJSON -Headers $authToken
...
Write-WELog " Adding App Protection Policies to Intune..." " INFO" -ForegroundColor Cyan
Write-Information Write-WELog " Adding iOS Managed App Policy from JSON..." " INFO"
Write-WELog " Creating Policy via Graph" " INFO"

$WECreateResult = Add-ManagedAppPolicy -Json $iOS
Write-Information " Policy created with id" $WECreateResult.id

$WEMAM_PolicyID = $WECreateResult.id

$WEAssign_Policy = Assign-ManagedAppPolicy -Id $WEMAM_PolicyID -TargetGroupId $WETargetGroupId -OS iOS
Write-WELog " Assigned '$WEAADGroup' to $($WECreateResult.displayName)/$($WECreateResult.id)" " INFO"

Write-Information write-host " Adding Android Managed App Policy from JSON..." -f Yellow
Write-WELog " Creating Policy via Graph" " INFO"

$WECreateResult = Add-ManagedAppPolicy -Json $WEAndroid
Write-Information " Policy created with id" $WECreateResult.id
; 
$WEMAM_PolicyID = $WECreateResult.id
; 
$WEAssign_Policy = Assign-ManagedAppPolicy -Id $WEMAM_PolicyID -TargetGroupId $WETargetGroupId -OS Android
Write-WELog " Assigned '$WEAADGroup' to $($WECreateResult.displayName)/$($WECreateResult.id)" " INFO"
