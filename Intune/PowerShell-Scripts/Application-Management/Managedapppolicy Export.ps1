...
<#
.SYNOPSIS
    We Enhanced Managedapppolicy Export
...
$graphApiVersion = " Beta"
    ...
    try {
        ...
        break
            ...
            $WEResource = " deviceAppManagement/androidManagedAppProtections('$id')/?`$expand=apps"
            ...
            $WEResource = " deviceAppManagement/iosManagedAppProtections('$id')/?`$expand=apps"
...
Write-Information write-host " Running query against Microsoft Graph for App Protection Policies" -f Yellow

$WEManagedAppPolicies = Get-ManagedAppPolicy -ErrorAction Stop | ? { ($_.'@odata.type').contains(" ManagedAppProtection" ) }

Write-Information if($WEManagedAppPolicies){

    foreach($WEManagedAppPolicy in $WEManagedAppPolicies){

    Write-Information " Managed App Policy:" $WEManagedAppPolicy.displayName -f Cyan

        if($WEManagedAppPolicy.'@odata.type' -eq " #microsoft.graph.androidManagedAppProtection" ){

           ;  $WEAppProtectionPolicy = Get-ManagedAppProtection -id $WEManagedAppPolicy.id -OS " Android"

            $WEAppProtectionPolicy | Add-Member -MemberType NoteProperty -Name '@odata.type' -Value " #microsoft.graph.androidManagedAppProtection"

            $WEAppProtectionPolicy

            Export-JSONData -JSON $WEAppProtectionPolicy -ExportPath " $WEExportPath"

        }

        elseif($WEManagedAppPolicy.'@odata.type' -eq " #microsoft.graph.iosManagedAppProtection" ){

           ;  $WEAppProtectionPolicy = Get-ManagedAppProtection -id $WEManagedAppPolicy.id -OS " iOS"

            $WEAppProtectionPolicy | Add-Member -MemberType NoteProperty -Name '@odata.type' -Value " #microsoft.graph.iosManagedAppProtection"
