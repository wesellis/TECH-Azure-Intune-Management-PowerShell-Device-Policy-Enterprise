<#
.SYNOPSIS
    Managedapppolicy Import Fromjson
...
<#
.SYNOPSIS
    We Enhanced Managedapppolicy Import Fromjson
...
$WEJSON_Convert = $WEJSON_Data | ConvertFrom-Json | Select-Object -Property * -ExcludeProperty id,createdDateTime,lastModifiedDateTime,version," @odata.context" ,apps@odata.context,deployedAppCount

$WEJSON_Apps = $WEJSON_Convert.apps | select * -ExcludeProperty id,version

$WEJSON_Convert | Add-Member -MemberType NoteProperty -Name 'apps' -Value @($WEJSON_Apps) -Force
; 
$WEDisplayName = $WEJSON_Convert.displayName
; 
$WEJSON_Output = $WEJSON_Convert | ConvertTo-Json -Depth 5
            
Write-Information Write-Information " App Protection Policy '$WEDisplayName' Found..."
Write-Information $WEJSON_Output
Write-Information Write-WELog " Adding App Protection Policy '$WEDisplayName'" " INFO"
Add-ManagedAppPolicy -JSON $WEJSON_Output
