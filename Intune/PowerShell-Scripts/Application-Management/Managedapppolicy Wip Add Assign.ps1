<#
.SYNOPSIS
    Managedapppolicy Wip Add Assign
...
<#
.SYNOPSIS
    We Enhanced Managedapppolicy Wip Add Assign
...
.EXAMPLE
Add-WindowsInformationProtectionPolicy -JSON $WEJSON
...
$WEResource = " deviceAppManagement/windowsInformationProtectionPolicies"
    
    try {
        
        if($WEJSON -eq "" -or $null -eq $WEJSON){

        Write-Information " No JSON specified, please specify valid JSON for the iOS Policy..." -f Red
...
The function connects to the Graph API Interface and adds a Windows Information Protection policy
.EXAMPLE
Add-MDMWindowsInformationProtectionPolicy -JSON $WEJSON
Adds a Windows Information Protection policy in Intune
.NOTES
...
.EXAMPLE
Assign-WindowsInformationProtectionPolicy -Id $WEId -TargetGroupId $WETargetGroupId
...
" @
    ...
    $uri = " https://graph.microsoft.com/beta/deviceAppManagement/windowsInformationProtectionPolicies('$WEID')/assign"
...
The function connects to the Graph API Interface and assigns a WIP App Policy with an AAD Group
.EXAMPLE
Assign-MDMWindowsInformationProtectionPolicy -Id $WEId -TargetGroupId $WETargetGroupId
Assigns an AAD Group assignment to a WIP App Protection Policy in Intune
.NOTES
...
" @
    ...
    $uri = " https://graph.microsoft.com/beta/deviceAppManagement/mdmWindowsInformationProtectionPolicies('$WEID')/assign"
    Invoke-RestMethod -Uri $uri -Method Post -ContentType " application/json" -Body $WEJSON -Headers $authToken
...
.EXAMPLE
Get-EnterpriseDomain -ErrorAction Stop
Gets the initial domain created in Azure - Format domain.onmicrosoft.com
.NOTES
NAME: Get-EnterpriseDomain -ErrorAction Stop
    ...
    $WEEnterpriseDomain = ($domains | ? { $_.isInitial -eq $true } | select id).id

    return $WEEnterpriseDomain
...
Write-Information $WEEnterpriseDomain = Get-EnterpriseDomain -ErrorAction Stop

$WESharepoint = $WEEnterpriseDomain.Split(" ." )[0]



$WEManagedAppPolicy_WIP = @"

{
  " description" : "" ,
  " displayName" : " Win10 WIP WE Policy - Assigned" ,
  " enforcementLevel" : " encryptAuditAndPrompt" ,
  " enterpriseDomain" : " $WEEnterpriseDomain" ,
  ...
  " enterpriseProxiedDomains" : [
    {
      ...
      " proxiedDomains" : [
        {
          " ipAddressOrFQDN" : " $WESharepoint.sharepoint.com" ,
          " @odata.type" : " #microsoft.graph.proxiedDomain"
        },
        {
          " ipAddressOrFQDN" : " $WESharepoint-my.sharepoint.com" ,
          " @odata.type" : " #microsoft.graph.proxiedDomain"
        },
        {
          " ipAddressOrFQDN" : " $WESharepoint.sharepoint.com" ,
...
Write-WELog " Adding Windows Information Protection Without Enrollment Policy from JSON..." " INFO" -ForegroundColor Yellow

$WECreateResult = Add-WindowsInformationProtectionPolicy -JSON $WEManagedAppPolicy_WIP

Write-Information " Policy created with id" $WECreateResult.id

$WEWIP_PolicyID = $WECreateResult.id

$WEAssign_Policy = Assign-WindowsInformationProtectionPolicy -Id $WEWIP_PolicyID -TargetGroupId $WETargetGroupId
Write-WELog " Assigned '$WEAADGroup' to $($WECreateResult.displayName)/$($WECreateResult.id)" " INFO"

Write-Information Write-WELog " Adding Windows Information Protection MDM Policy from JSON..." " INFO"

$WECreateResult = Add-MDMWindowsInformationProtectionPolicy -JSON $WEManagedAppPolicy_WIP_MDM

Write-Information " Policy created with id" $WECreateResult.id
; 
$WEWIP_PolicyID = $WECreateResult.id
; 
$WEAssign_Policy = Assign-MDMWindowsInformationProtectionPolicy -Id $WEWIP_PolicyID -TargetGroupId $WETargetGroupId
Write-WELog " Assigned '$WEAADGroup' to $($WECreateResult.displayName)/$($WECreateResult.id)" " INFO"
