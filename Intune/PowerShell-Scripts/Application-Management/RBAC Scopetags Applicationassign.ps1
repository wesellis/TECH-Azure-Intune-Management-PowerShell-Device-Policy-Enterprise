<#
.SYNOPSIS
    Rbac Scopetags Applicationassign
...
<#
.SYNOPSIS
    We Enhanced Rbac Scopetags Applicationassign
...
$WEResource = " deviceAppManagement/mobileApps/$id"
    ...
    try {
        ...
        if(($WEType -eq " #microsoft.graph.microsoftStoreForBusinessApp" ) -or ($WEType -eq " #microsoft.graph.iosVppApp" )){
...
Write-Information ; 
$WEScopeTags = (Get-RBACScopeTag).displayName | Sort-Object

if($WEScopeTags){

Write-WELog " Please specify Scope Tag you want to add to all users / devices in AAD Group:" " INFO" -ForegroundColor Yellow
; 
$menu = @{}
...
$selection = $menu.Item([int]$ans)
    ...
    if($selection){
        ...
        $WEScopeTagId = (Get-RBACScopeTag -ErrorAction Stop | Where-Object { $_.displayName -eq " $selection" }).id
...
if(@($WEApplication).count -eq 1){
    ...
    Write-WELog " Intune Application '$WEADN' with type '$WEAT' found..." " INFO"
        ...
        if($WEIA.roleScopeTagIds){

            if(!($WEIA.roleScopeTagIds).contains(" $WEScopeTagId" )){

                $WEST = @($WEIA.roleScopeTagIds) + @(" $WEScopeTagId" )

                $WEResult = Update-IntuneApplication -id $WEIA.id -Type $WEIA.'@odata.type' -ScopeTags $WEST

                if($WEResult -eq "" ){

                    Write-WELog " Intune Application '$WEADN' patched with ScopeTag '$selection'..." " INFO" -ForegroundColor Green
                ...
                Write-WELog " Scope Tag '$selection' already assigned to '$WEADN'..." " INFO" -ForegroundColor Magenta
...
elseif(@($WEApplication).count -gt 1){
    ...
    Write-WELog " No Intune Application found with '$displayName'..." " INFO" -ForegroundColor Red
