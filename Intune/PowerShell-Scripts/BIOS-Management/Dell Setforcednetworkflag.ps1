...
<#
.SYNOPSIS
    We Enhanced Dell Setforcednetworkflag
...
param(
    ...
    PROCESS {
        if ($WEValue -ne "" )
        {
            ...
            $bytes = $enc.GetBytes($WEValue)
            Write-Verbose " Setting variable $WEVariableName to a string value with $($bytes.Length) characters"
            $rc = $uefiNative[0]::SetFirmwareEnvironmentVariableA($WEVariableName, $WENamespace, $bytes, $bytes.Length)
        }
        else
        {
            Write-Verbose " Setting variable $WEVariableName to a byte array with $($WEByteArray.Length) bytes"
            $rc = $uefiNative[0]::SetFirmwareEnvironmentVariableA($WEVariableName, $WENamespace, $WEByteArray, $WEByteArray.Length)
        }
        if ($rc -eq 0)
        {
            Write-Error " Unable to set variable $WEVariableName from namespace $WENamespace"
        }
    }
    END {
