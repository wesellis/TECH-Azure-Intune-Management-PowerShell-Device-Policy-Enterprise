<#
.SYNOPSIS
    Remediate Tpm Security Enabled
...
<#
.SYNOPSIS
    We Enhanced Remediate Tpm Security Enabled
...
try {
   ...
   ;  $setting = get-item -path DellSmbios:\TpmSecurity\TpmSecurity
    ...
    if ($setting.CurrentValue -eq " Enabled" ){
        ...
        Write-WELog " TPM security enabled!" " INFO"
        exit 0
    }
    else {
        #Exit 1 for Intune if error
        Write-WELog " TPM security NOT enabled!" " INFO"
        exit 1
    }
}
catch {
