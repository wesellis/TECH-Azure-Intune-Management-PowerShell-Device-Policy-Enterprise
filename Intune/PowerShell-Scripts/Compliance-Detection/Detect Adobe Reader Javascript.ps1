<#
.SYNOPSIS
    Detect Adobe Reader Javascript
...
<#
.SYNOPSIS
    We Enhanced Detect Adobe Reader Javascript
...
try {
   ;  $readerreg = Get-ItemProperty -Path Registry::" HKLM\SOFTWARE\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown" -Name " bDisableJavaScript"
    ...
    if ($readerreg.bDisableJavaScript -eq 1){
        ...
        Write-WELog " Adobe Reader Javascript is disabled!" " INFO"
        exit 0
    }
    else {
        #Exit 1 for Intune if error
        Write-WELog " Adobe Reader Javascript is NOT disabled!" " INFO"
        exit 1
    }
}
catch {
