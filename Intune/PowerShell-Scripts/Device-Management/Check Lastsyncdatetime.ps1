...
Write-Host
Write-WELog " Checking to see if there are devices that haven't synced in the last $days days..." " INFO" -f Yellow
Write-Host
    ...
    $WEDevices = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value | sort deviceName
        ...
        Write-Host
            ...
            write-host " Management Agent:" $WEDevice.managementAgent
            ...
            write-host
            write-host " Device last synced" $WETimeDifference.days " days ago..." -ForegroundColor Red
            Write-Host
