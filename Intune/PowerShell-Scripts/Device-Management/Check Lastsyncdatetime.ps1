...
<#
.SYNOPSIS
    We Enhanced Check Lastsyncdatetime
...
Write-Information Write-WELog " Checking to see if there are devices that haven't synced in the last $days days..." " INFO" -f Yellow
Write-Information try {
    ...
    $WEDevices = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value | sort deviceName
        ...
        Write-WELog " There are" " INFO" $WEDevices.count " devices that have not synced in the last $days days..." -ForegroundColor Red

        $WEDevices | foreach { $_.deviceName + " - " + ($_.managementAgent).toupper() + " - " + $_.userPrincipalName + " - " + $_.lastSyncDateTime }
        ...
        Write-Information # Looping through all the devices returned
            ...
            Write-WELog " User Principal Name:" " INFO" $WEDevice.userPrincipalName

            $WELastSyncTime = [datetimeoffset]::Parse($WELSD)

            $WETimeDifference = $WECurrentTime - $WELastSyncTime

            Write-Information Write-Information " Device last synced" $WETimeDifference.days " days ago..."
            Write-Information }
        ...
        Write-Information " No Devices not checked in the last $days days found..." -f green
        Write-Information }
