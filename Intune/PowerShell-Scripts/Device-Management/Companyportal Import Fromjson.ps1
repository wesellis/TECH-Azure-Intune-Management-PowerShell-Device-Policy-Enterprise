...
<#
.SYNOPSIS
    We Enhanced Companyportal Import Fromjson
...
.EXAMPLE
Set-IntuneBrand -JSON $WEJSON
Sets the Company Intune Brand using Graph API
.NOTES
NAME: Set-IntuneBrand -ErrorAction Stop
...
$WEApp_resource = " deviceManagement"
...
$WEImportPath = Read-Host -Prompt " Please specify a path to a JSON file to import data from e.g. C:\IntuneOutput\Branding\Branding.json"
...
$WEJSON_Convert = $WEJSON_Data | ConvertFrom-Json | Select-Object -Property * -ExcludeProperty '@odata.context'

$WEDisplayName = $WEJSON_Convert.displayName
; 
$WEIntuneBrand = @(@{intuneBrand=$WEJSON_Convert})
; 
$WEJSON_Output = $WEIntuneBrand | ConvertTo-Json -Depth 5

Write-Information Write-Information " Company Portal Branding '$WEDisplayName' Found..."
Write-Information $WEJSON_Output
Write-Information Write-WELog " Setting Company Portal Branding '$WEDisplayName'" " INFO"
Set-IntuneBrand -JSON $WEJSON_Output
