...
<#
.SYNOPSIS
    We Enhanced Corpdeviceenrollment Remove
...
$graphApiVersion = " beta"
    ...
    try {
        ...
        if($WEDeviceIdentifier){

            $WEResource = " deviceManagement/importedDeviceIdentities?`$filter=contains(importedDeviceIdentifier,'$WEDeviceIdentifier')"
...
$graphApiVersion = " beta"
$WEResource = " deviceManagement/importedDeviceIdentities/$WEImportedDeviceId"
...
$WECDI = Get-CorporateDeviceIdentifiers -DeviceIdentifier " 123456789012345"

if(@($WECDI).count -eq 1){

   ;  $WECDI_Id = $WECDI.id
   ;  $WECDI_Identifier = $WECDI.importedDeviceIdentifier
