...
<#
.SYNOPSIS
    We Enhanced Deviceconfiguration Export
...
Write-Information ; 
$WEDCPs = Get-DeviceConfigurationPolicy -ErrorAction Stop | Where-Object { ($_.'@odata.type' -ne " #microsoft.graph.iosUpdateConfiguration" ) -and ($_.'@odata.type' -ne " #microsoft.graph.windowsUpdateForBusinessConfiguration" ) }
foreach($WEDCP in $WEDCPs){

Write-Information " Device Configuration Policy:" $WEDCP.displayName -f Yellow
Export-JSONData -JSON $WEDCP -ExportPath " $WEExportPath"
Write-Information }
