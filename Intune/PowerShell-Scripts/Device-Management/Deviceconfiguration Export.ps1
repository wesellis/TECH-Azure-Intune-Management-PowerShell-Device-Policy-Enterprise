...
; 
$WEDCPs = Get-DeviceConfigurationPolicy | Where-Object { ($_.'@odata.type' -ne " #microsoft.graph.iosUpdateConfiguration" ) -and ($_.'@odata.type' -ne " #microsoft.graph.windowsUpdateForBusinessConfiguration" ) }
foreach($WEDCP in $WEDCPs){

write-host " Device Configuration Policy:" $WEDCP.displayName -f Yellow
