...
<#
.SYNOPSIS
    We Enhanced Deviceconfiguration Get
...
[string]$WEAllUsers   = " acacacac-9df4-4c7d-9d50-4ef0226f57a9"
[string]$WEAllDevices = " adadadad-808e-44e2-905a-0b7873a8a531"
    ...
    try {
       ...
       ;  $uri = " https://graph.microsoft.com/$graphApiVersion/$($WEGroup_resource)?`$filter=id eq '$id'"
        switch ( $id ) {
                $WEAllUsers   {;  $grp = [PSCustomObject]@{ displayName = " All users" }; $grp           }
                $WEAllDevices { $grp = [PSCustomObject]@{ displayName = " All devices" }; $grp         }
...
Write-Information " Device Configuration Policy:" $WEDCP.displayName -f Yellow
Write-Information $WEDCP
; 
$id = $WEDCP.id
; 
$WEDCPA = Get-DeviceConfigurationPolicyAssignment -id $id
Write-Information " Getting Configuration Policy assignment..." -f Cyan
    if($WEDCPA){
    if($WEDCPA.count -gt 1){
        ...
        (Get-AADGroup -id $WEDCPA.targetGroupId).displayName

        }

    }
    else {
        Write-WELog " No assignments found." " INFO"
    }
    Write-Information }
