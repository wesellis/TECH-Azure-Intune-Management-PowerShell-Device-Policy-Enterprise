...
<#
.SYNOPSIS
    We Enhanced Deviceconfiguration Remove
...
.NOTES
NAME: Remove-DeviceConfigurationPolicy -ErrorAction Stop
...
$WEDCP_resource = " deviceManagement/deviceConfigurations"
    ...
    try {
        ...
        Write-Information " No id specified for device configuration, can't remove configuration..." -f Red
        Write-Information " Please specify id for device configuration..." -f Red
        break
...
; 
$WECP = Get-DeviceConfigurationPolicy -name " Test Graph Policy"

    if($WECP){

        if(@($WECP).count -gt 1){

        Write-WELog " More than one device configuration policy has been found, please specify a single device configuration policy..." " INFO" -ForegroundColor Red
        Write-Information }

        elseif(@($WECP).count -eq 1){

        Write-WELog " Removing device configuration policy" " INFO" $WECP.displayName -ForegroundColor Yellow
        Remove-DeviceConfigurationPolicy -id $WECP.id
    ...
    Write-WELog " Device Configuration Policy doesn't exist..." " INFO"
    Write-Information }
