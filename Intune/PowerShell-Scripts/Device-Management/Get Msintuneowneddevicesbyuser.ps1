...
<#
.SYNOPSIS
    We Enhanced Get Msintuneowneddevicesbyuser
...
Process {
    # Define resources
    $WEGraphURI = " https://graph.microsoft.com/beta/users/$($WEUserName)/ownedDevices"
    ...
    if ($WEOwnedDevices -ne $null) {
        foreach ($WEObject in $WEOwnedDevices) {
           ...
           ;  $WEPSObject = [PSCustomObject]@{
                ...
                DeviceName = $WEObject.displayName
                Id = $WEObject.deviceId
                Compliant = $WEObject.isCompliant
                Managed = $WEObject.isManaged
                OS = $WEObject.operatingSystem
                OSVersion = $WEObject.operatingSystemVersion
            }
