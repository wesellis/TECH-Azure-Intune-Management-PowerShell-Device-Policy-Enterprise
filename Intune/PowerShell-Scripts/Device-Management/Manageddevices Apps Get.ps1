...
<#
.SYNOPSIS
    We Enhanced Manageddevices Apps Get
...
$WEDevices = Get-ManagedDevices -ErrorAction Stop
...
if($WEDevices){
    ...
    Write-Information if($WEDevice.ownerType -eq " personal" ){

        Write-WELog " Device Ownership:" " INFO" $WEDevice.ownerType -ForegroundColor Cyan

        }

        elseif($WEDevice.ownerType -eq " company" ){

        Write-WELog " Device Ownership:" " INFO" $WEDevice.ownerType -ForegroundColor Magenta

        }

   ;  $uri = " https://graph.microsoft.com/beta/deviceManagement/manageddevices('$WEDeviceID')?`$expand=detectedApps"
   ;  $WEDetectedApps = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).detectedApps

    $WEDetectedApps | select displayName,version | ft
