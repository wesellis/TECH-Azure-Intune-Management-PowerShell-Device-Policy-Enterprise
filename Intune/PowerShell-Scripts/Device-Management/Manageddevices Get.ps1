...
<#
.SYNOPSIS
    We Enhanced Manageddevices Get
...
if($WEManagedDevices){
    ...
    foreach($WEDevice in $WEManagedDevices){

    $WEDeviceID = $WEDevice.id

    Write-Information " Managed Device" $WEDevice.deviceName " found..." -ForegroundColor Yellow
    Write-Information $WEDevice

        if($WEDevice.deviceRegistrationState -eq " registered" ){

       ;  $WEUserId = $WEDevice.userId

       ;  $WEUser = Get-AADUser -ErrorAction Stop $userId

        Write-WELog " Device Registered User:" " INFO" $WEUser.displayName -ForegroundColor Cyan
        Write-WELog " User Principle Name:" " INFO" $WEUser.userPrincipalName
