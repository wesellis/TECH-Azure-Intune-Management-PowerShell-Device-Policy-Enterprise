<#
.SYNOPSIS
    Manageddevices Ios Passcodereset Export
...
<#
.SYNOPSIS
    We Enhanced Manageddevices Ios Passcodereset Export
...
$WEResource = " deviceManagement/reports/exportJobs"
    ...
    $WEJson = ConvertTo-Json -InputObject $psObj
    ...
    if($WEName){
        ...
        $uri = " https://graph.microsoft.com/$graphApiVersion/$($WEResource)"
        $result = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Post -Body $WEJSON -ContentType " application/json" )

        $id = $result.id

        Write-Information " Export Job id is '$id'"

        Write-Information while($true){

                $pollingUri = " $uri('$id')"
                Write-Information " Polling uri = " $pollingUri
               ...
               ;  $status = $result.status
                ...
                if ($status -eq 'completed'){
                    ...
                    Write-WELog " Export Job Complete..." " INFO" -ForegroundColor Green
                    Write-Information ;  $fileName = (Split-Path -Path $result.url -Leaf).split('?')[0]

                    Invoke-WebRequest -Uri $result.url -OutFile $env:temp\$fileName

                    Write-Information " Downloaded Export to local disk as '$env:temp\$fileName'..."
                    Write-Information break;
                    ...
                    Write-WELog " In progress, waiting..." " INFO" -ForegroundColor Yellow
