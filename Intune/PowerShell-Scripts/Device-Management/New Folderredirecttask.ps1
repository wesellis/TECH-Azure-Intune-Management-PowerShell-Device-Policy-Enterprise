...
<#
.SYNOPSIS
    We Enhanced New Folderredirecttask
...
param(
    [Parameter()] $WEUrl = " https://raw.githubusercontent.com/aaronparker/intune/main/Folder-Redirection/Redirect-Folders.ps1" ,
    [Parameter()] $WEScript = " Redirect-Folders.ps1" ,
    [Parameter()] $WEScriptVb = " Redirect-Folders.vbs" ,
    [Parameter()] $WETaskName = " Folder Redirection" ,
    [Parameter()] $WEGroup = $((Get-LocalGroup -SID S-1-5-32-545).Name),
...
Else {
   ...
   ;  $principal = New-ScheduledTaskPrincipal -GroupId $WEGroup
