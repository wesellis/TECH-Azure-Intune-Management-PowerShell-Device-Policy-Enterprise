...
$WEMemberGroupId = (get-AADGroup -GroupName " $WEMemberAADGroup" ).id
    ...
    if($WEMemberGroupId -eq $null -or $WEMemberGroupId -eq "" ){
...
$WEAADGroup = Read-Host -Prompt " Enter the Azure AD Group name for Intune Role Scope"
...
write-host " Intune Role created with id" $WECreateResult.id
; 
$WEIntuneRoleID = $WECreateResult.id

Write-Host

Write-WELog " Creating Intune Role Assignment..." " INFO" -ForegroundColor Yellow
...
write-host " Intune Role Assigment created with id" $WEAssignmentIntuneRole.id
