...
<#
.SYNOPSIS
    We Enhanced Rbac Scopetags Policyassign
...
Write-WELog " Are you sure you want to add Scope Tags to all Configuration and Compliance Policies? Y or N?" " INFO"
$WEConfirm = read-host

if($WEConfirm -eq " y" -or $WEConfirm -eq " Y" ){

    Write-WELog " Checking if any Scope Tags have been created..." " INFO"
    Write-Information $WEScopeTags = Get-RBACScopeTag -ErrorAction Stop

    if($WEScopeTags){

        Write-WELog " Scope Tags found..." " INFO" -ForegroundColor Green
        Write-Information foreach($WEScopeTag in $WEScopeTags){

        $WEScopeTag_DN = $WEScopeTag.displayName
        $WEScopeTagId = $WEScopeTag.id

        Write-WELog " Checking Scope Tag '$WEScopeTag_DN'..." " INFO" -ForegroundColor Cyan

        ####################################################
        
        #region Device Compliance Policies

        Write-WELog " Testing if '$WEScopeTag_DN' exists in Device Compliance Policy Display Name..." " INFO"

        $WECPs = Get-DeviceCompliancePolicy -ErrorAction Stop | Where-Object { ($_.displayName).contains(" $WEScopeTag_DN" ) } | Sort-Object displayName

            if($WECPs){

                foreach($WEPolicy in $WECPs){
        
                    $WECP = Get-DeviceCompliancePolicy -id $WEPolicy.id

                    $WECP_DN = $WECP.displayName

                    if($WECP.roleScopeTagIds){

                        if(!($WECP.roleScopeTagIds).contains(" $WEScopeTagId" )){

                            $WEST = @($WECP.roleScopeTagIds) + @(" $WEScopeTagId" )

                            $WEResult = Update-DeviceCompliancePolicy -id $WEPolicy.id -Type $WEPolicy.'@odata.type' -ScopeTags $WEST

                            if($WEResult -eq "" ){

                                Write-WELog " Compliance Policy '$WECP_DN' patched with '$WEScopeTag_DN' ScopeTag..." " INFO" -ForegroundColor Green
                            ...
                            Write-WELog " Scope Tag '$WEScopeTag_DN' already assigned to '$WECP_DN'..." " INFO" -ForegroundColor Red
            ...
            Write-Information #endregion

        ####################################################

        #region Device Configuration Policies

        Write-WELog " Testing if '$WEScopeTag_DN' exists in Device Configuration Policy Display Name..." " INFO"

        $WEDCPs = Get-DeviceConfigurationPolicy -ErrorAction Stop | Where-Object { ($_.displayName).contains(" $WEScopeTag_DN" ) } | Sort-Object displayName

            if($WEDCPs){

                foreach($WEPolicy in $WEDCPs){
        
                    $WEDCP = Get-DeviceConfigurationPolicy -id $WEPolicy.id

                    $WEDCP_DN = $WEDCP.displayName

                    if($WEDCP.roleScopeTagIds){

                        if(!($WEDCP.roleScopeTagIds).contains(" $WEScopeTagId" )){

                            $WEST = @($WEDCP.roleScopeTagIds) + @(" $WEScopeTagId" )

                            $WEResult = Update-DeviceConfigurationPolicy -id $WEPolicy.id -Type $WEPolicy.'@odata.type' -ScopeTags $WEST

                            if($WEResult -eq "" ){

                                Write-WELog " Configuration Policy '$WEDCP_DN' patched with '$WEScopeTag_DN' ScopeTag..." " INFO" -ForegroundColor Green
                            ...
                            Write-WELog " Scope Tag '$WEScopeTag_DN' already assigned to '$WEDCP_DN'..." " INFO" -ForegroundColor Red
        ...
        Write-WELog " No Scope Tags configured..." " INFO" -ForegroundColor Red
    ...
    Write-WELog " Addition of Scope Tags to all Configuration and Compliance Policies was cancelled..." " INFO" -ForegroundColor Yellow
