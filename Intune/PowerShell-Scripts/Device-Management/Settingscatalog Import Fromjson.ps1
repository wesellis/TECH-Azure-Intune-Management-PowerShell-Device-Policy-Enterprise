<#
.SYNOPSIS
    Settingscatalog Import Fromjson
...
<#
.SYNOPSIS
    We Enhanced Settingscatalog Import Fromjson
...
.EXAMPLE
Add-SettingsCatalogPolicy -JSON $WEJSON
...
$WEResource = " deviceManagement/configurationPolicies"

    try {

        if($WEJSON -eq "" -or $null -eq $WEJSON){

        Write-Information " No JSON specified, please specify valid JSON for the Endpoint Security Disk Encryption Policy..." -f Red
...
$WEJSON_Convert = $WEJSON_Data | ConvertFrom-Json | Select-Object -Property * -ExcludeProperty id,createdDateTime,lastModifiedDateTime,version,supportsScopeTags
; 
$WEDisplayName = $WEJSON_Convert.name
; 
$WEJSON_Output = $WEJSON_Convert | ConvertTo-Json -Depth 20
            
Write-Information Write-Information " Settings Catalog Policy '$WEDisplayName' Found..."
Write-Information $WEJSON_Output
Write-Information Write-WELog " Adding Settings Catalog Policy '$WEDisplayName'" " INFO"
Add-SettingsCatalogPolicy -JSON $WEJSON_Output
