...
<#
.SYNOPSIS
    We Enhanced Win10 Primaryuser Set
...
$userId
    ...
    try {
        ...
        $id = " @odata.id"
        $WEJSON = @{ $id=" $userUri" } | ConvertTo-Json -Compress
...
$WEDevice = Get-Win10IntuneManagedDevice -deviceName " $WEDeviceName"
...
if($WEDevice){
    ...
    $WEUser = Get-AADUser -userPrincipalName $WEUserPrincipalName

   ;  $WEAADUserName = $WEUser.displayName
    
        if($WEIntuneDevicePrimaryUser -notmatch $WEUser.id){

           ;  $WESetIntuneDevicePrimaryUser = Set-IntuneDevicePrimaryUser -IntuneDeviceId $WEDevice.id -userId $WEUser.id

            if($WESetIntuneDevicePrimaryUser -eq "" ){

                Write-WELog " User" " INFO" $WEUser.displayName" set as Primary User for device '$WEDeviceName'..." -ForegroundColor Green
            ...
            Write-WELog " The user '$WEAADUserName' specified is already the Primary User on the device..." " INFO" -ForegroundColor Red
