...
<#
.SYNOPSIS
    We Enhanced Appconfigurationpolicy Export
...
.EXAMPLE
Get-ManagedAppAppConfigPolicy -ErrorAction Stop
Returns any app configuration policy for managed apps configured in Intune
.NOTES
NAME: Get-ManagedAppAppConfigPolicy -ErrorAction Stop


$graphApiVersion = " Beta"
$WEResource = " deviceAppManagement/targetedManagedAppConfigurations?`$expand=apps"
...
.EXAMPLE
Get-ManagedDeviceAppConfigPolicy -ErrorAction Stop
Returns any app configuration policy for managed devices configured in Intune
.NOTES
NAME: Get-ManagedDeviceAppConfigPolicy -ErrorAction Stop
...
.NOTES
NAME: Get-AppBundleID -ErrorAction Stop
...
$graphApiVersion = " Beta"
$WEResource = " deviceAppManagement/mobileApps?`$filter=id eq '$WEGUID'"
...
$bundleID
    ...
    try {
        ...
        $WEProperties = ($WEJSON_Convert | Get-Member -ErrorAction Stop | ? { $_.MemberType -eq " NoteProperty" }).Name
            ...
            $WEFileName_JSON = " $WEDisplayName" + " _" + $(get-date -f dd-MM-yyyy-H-mm-ss) + " 1.json"
            ...
            $WEObject = New-Object -ErrorAction Stop System.Object
                ...
                If($bundleID)
                {
                    ...
                    $WEObject | Add-Member -MemberType NoteProperty -name " bundleID" -Value $bundleID

                }

            Write-Information " Export Path:" " $WEExportPath"
        
            $object | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath " $WEExportPath\$WEFileName_JSON"
...
Write-Information $managedAppAppConfigPolicies = Get-ManagedAppAppConfigPolicy -ErrorAction Stop

    foreach($policy in $managedAppAppConfigPolicies){

    Write-Information " (Managed App) App Configuration Policy:" $policy.displayName -f Yellow
    Export-JSONData -JSON $policy -ExportPath " $WEExportPath"
    Write-Information }
; 
$managedDeviceAppConfigPolicies = Get-ManagedDeviceAppConfigPolicy -ErrorAction Stop

    foreach($policy in $managedDeviceAppConfigPolicies){

    Write-Information " (Managed Device) App Configuration  Policy:" $policy.displayName -f Yellow
        ...
        If($policy.'@odata.type' -eq " #microsoft.graph.iosMobileAppConfiguration" ){
           ...
           ;  $bundleID = Get-AppBundleID -GUID $policy.targetedMobileApps
            Export-JSONData -JSON $policy -ExportPath " $WEExportPath" -bundleID $bundleID.bundleID
            Write-Information }
