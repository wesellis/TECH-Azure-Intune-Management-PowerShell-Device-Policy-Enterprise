...
<#
.SYNOPSIS
    We Enhanced Compliancepolicy Remove
...
$WEResource = " deviceManagement/deviceCompliancePolicies"
    ...
    try {
        ...
        Write-Information " No id specified for device compliance, can't remove compliance policy..." -f Red
        Write-Information " Please specify id for device compliance policy..." -f Red
        break
...
; 
$WECP = Get-DeviceCompliancePolicy -Name " Test Policy"

    if($WECP){

        if(@($WECP).count -gt 1){

        Write-WELog " More than one compliance policy has been found, please specify a single compliance policy..." " INFO" -ForegroundColor Red
        Write-Information }

        elseif(@($WECP).count -eq 1){

        Write-WELog " Removing compliance policy" " INFO" $WECP.displayName -ForegroundColor Yellow
        Remove-DeviceCompliancePolicy -id $WECP.id
