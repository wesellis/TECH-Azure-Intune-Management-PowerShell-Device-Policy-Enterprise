<#
.SYNOPSIS
    Remediate Dell Bios Auto Power
...
<#
.SYNOPSIS
    We Enhanced Remediate Dell Bios Auto Power
...
try {
   ...
   ;  $autoonmn = get-Item -Path DellSmbios:\PowerManagement\AutoOnMn
    ...
    if ($autoon.CurrentValue -eq 'Everyday' -AND $autoonhr.CurrentValue -eq 21 -AND $autoonmn.CurrentValue -eq 00){
        ...
        Write-WELog " Successfully set Auto-on to everyday, 9PM!" " INFO"
        exit 0
    }
    else {
        #Exit 1 for Intune if error
        Write-WELog " Failed to set Auto-on!" " INFO"
        exit 1
    }
}
catch {
