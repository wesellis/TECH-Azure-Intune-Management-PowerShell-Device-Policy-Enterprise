...
; 
$WEAuditCategories = Get-AuditCategories

Write-WELog " Intune Audit Categories:" " INFO" -ForegroundColor Yellow
; 
$menu = @{}

for ($i=1;$i -le $WEAuditCategories.count; $i++) 
{ Write-WELog " $i. $($WEAuditCategories[$i-1])" " INFO" 
$menu.Add($i,($WEAuditCategories[$i-1]))}
...
Write-Host
; 
$WEEvents = Get-AuditEvents -Category $selection

    if($WEEvents){

        foreach($WEEvent in ($WEEvents | Sort-Object -Property activityDateTime )){

        Write-Host $WEEvent.displayName -f Yellow
        ...
        Write-Host
        Write-WELog " User Information" " INFO" -ForegroundColor Cyan
        $WEEvent.actor
    ...
    Write-WELog " No audit events found for '$selection' in the past month..." " INFO" -ForegroundColor Cyan
    Write-Host
    Write-WELog " -------------------------------------------------------------------" " INFO"
    Write-Host
