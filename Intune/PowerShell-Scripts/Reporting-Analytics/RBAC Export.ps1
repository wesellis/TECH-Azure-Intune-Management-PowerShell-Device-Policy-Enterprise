...
$WEExportPath = Read-Host -Prompt " Please specify a path to export RBAC Intune Roles to e.g. C:\IntuneOutput"
...
Write-Information $WERBAC_Roles = (Get-RBACRole -ErrorAction Stop | Where-Object { $_.isBuiltIn -eq $false })

foreach($WERBAC_Role in $WERBAC_Roles){

    $WERBAC_DisplayName = $WERBAC_Role.displayName
    $WERBAC_Description = $WERBAC_Role.description

    $WEFileName_JSON = " $WERBAC_DisplayName" + " _" + $(get-date -f dd-MM-yyyy-H-mm-ss) + " .json"

   ;  $WERBAC_Actions = $WERBAC_Role.RolePermissions.resourceActions.allowedResourceActions | ConvertTo-Json
; 
$WEJSON = @"
{
    " @odata.type" : " #microsoft.graph.deviceAndAppManagementRoleDefinition" ,
    " displayName" : " $WERBAC_DisplayName" ,
    " description" : " $WERBAC_Description" ,
    " rolePermissions" : [
        {
        " resourceActions" : [
        {
        " allowedResourceActions" : $WERBAC_Actions,
...
" @
    ...
    $WEJSON | Set-Content -LiteralPath " $WEExportPath\$WEFileName_JSON"
