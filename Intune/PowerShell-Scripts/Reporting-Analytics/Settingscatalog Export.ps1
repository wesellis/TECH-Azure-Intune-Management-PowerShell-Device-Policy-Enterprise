...
<#
.SYNOPSIS
    We Enhanced Settingscatalog Export
...
.NOTES
NAME: Get-SettingsCatalogPolicySettings -ErrorAction Stop
...
$WEResource = " deviceManagement/configurationPolicies('$policyid')/settings?`$expand=settingDefinitions"
    ...
    try {
        ...
        while ($null -ne $WEResponseNextLink){
...
$WEExportPath
    ...
    try {
        ...
        elseif(!(Test-Path $WEExportPath)){
            ...
            $WEJSON1 = ConvertTo-Json $WEJSON -Depth 20

            $WEJSON_Convert = $WEJSON1 | ConvertFrom-Json

            $displayName = $WEJSON_Convert.name
...
$WEPolicies = Get-SettingsCatalogPolicy -ErrorAction Stop

if($WEPolicies){

    foreach($policy in $WEPolicies){

        Write-Information $policy.name -ForegroundColor Yellow
       ...
       ;  $WEInstances = $WESettingInstances.settingInstance
        ...
        Export-JSONData -JSON $WEPolicyBody -ExportPath " $WEExportPath"
        Write-Information }
