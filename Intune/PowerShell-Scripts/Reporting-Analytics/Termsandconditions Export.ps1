...
<#
.SYNOPSIS
    We Enhanced Termsandconditions Export
...
.EXAMPLE
Get-TermsAndConditions -ErrorAction Stop
Returns the Organization resource configured in Intune
.NOTES
NAME: Get-TermsAndConditions -ErrorAction Stop
...
param(
    ...
    try
    {
        ...
        else
        {
            ...
            Write-Information " CSV created in $WEExportPath\$WEFileName_CSV..." -f cyan
...
Write-Information ; 
$WETCs = Get-TermsAndConditions -ErrorAction Stop

foreach ($WETC in $WETCs)
{
	
	Write-Information " Terms and Conditions Policy:" $WETSc.displayName -f Yellow
	Export-JSONData -JSON $WETC -ExportPath " $WEExportPath"
	Write-Information }
