...
<#
.SYNOPSIS
    We Enhanced Endpointsecuritypolicy Import Fromjson
...
.EXAMPLE
Add-EndpointSecurityDiskEncryptionPolicy -JSON $WEJSON -TemplateId $templateId
...
$graphApiVersion = " Beta"
$WEESP_resource = " deviceManagement/templates/$WETemplateId/createInstance"
Write-Verbose " Resource: $WEESP_resource"
...
$WEJSON_Convert = $WEJSON_Data | ConvertFrom-Json


$WEJSON_DN = $WEJSON_Convert.displayName
...
$WEES_Template = $WETemplates | ?  { $_.id -eq $WEJSON_TemplateId }




if(($WEES_Template.templateType -eq " microsoftEdgeSecurityBaseline" ) -or ($WEES_Template.templateType -eq " securityBaseline" ) -or ($WEES_Template.templateType -eq " advancedThreatProtectionSecurityBaseline" )){

    $WETemplateId = $WEJSON_Convert.templateId
...
elseif($WEES_Template){

    # if template isn't deprecated use templateId
    if($WEES_Template.isDeprecated -eq $false){
    ...
    elseif($WEES_Template.isDeprecated -eq $true) {

        $WETemplate = $WETemplates | ? { $_.displayName -eq " $WEJSON_TemplateDisplayName" }

        $WETemplate = $WETemplate | ? { $_.isDeprecated -eq $false }

        $WETemplateId = $WETemplate.id
...
elseif($WEES_Template -eq $null){
    ...
    If($WEES_Template){
        ...
        if(($WEES_Template.templateType -eq " securityBaseline" ) -or ($WEES_Template.templateType -eq " advancedThreatProtectionSecurityBaseline" )){
            ...
            Write-WELog " Template with displayName '$WEJSON_TemplateDisplayName' found..." " INFO" -ForegroundColor Green

            $WETemplate = $WEES_Template | ? { $_.isDeprecated -eq $false }
...
$WEJSON_Convert = $WEJSON_Convert | Select-Object -Property * -ExcludeProperty TemplateDisplayName,TemplateId,versionInfo
; 
